# -*- coding: utf-8 -*-
import scrapy
from auto_scrapy.items import AutoScrapyItem
import urllib
...
class WangyiSpider(scrapy.Spider):
    ...
    start_urls = ['http://product.auto.163.com/rank/hotIndices_SUV.html#fixedpos']

    def parse(self, response):
        # suv的分类index
        suv_index = response.xpath('//*[@id="bd"]/div/div[1]/div[2]/div[2]/div[1]/ul[1]//li')
        autoItem = AutoScrapyItem()
        for suv_type in suv_index:
            # 拿到每种suv的链接，和text
            suv_url = suv_type.xpath('./a/@href').extract()[0]
            category = suv_type.xpath('./a/text()').extract()[0]       # 例如：‘十万以下SUV’
